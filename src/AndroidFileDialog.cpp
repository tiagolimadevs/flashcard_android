...
bool AndroidFileDialog::requestPermission() {
	QString request = "android.permission.READ_EXTERNAL_STORAGE";
	auto result = QtAndroid::requestPermissionsSync({request});
	if (result[request] == QtAndroid::PermissionResult::Granted)
		return true;
	return false;
}

bool AndroidFileDialog::isGranted() {
	QString request = "android.permission.READ_EXTERNAL_STORAGE";
	return QtAndroid::checkPermission(request) == QtAndroid::PermissionResult::Granted;
}
